...
namespace AwesomeApp.Web.Endpoints.ShoppingListEndpoints
{
        ...
        .WithResponse<ListIncompleteResponse>
    {
        ...
        [HttpGet("/ShoppingLists/{ProjectId}/IncompleteItems")]
        [SwaggerOperation(
            Summary = "Gets a list of a shoppingList's incomplete items",
            Description = "Gets a list of a shoppingList's incomplete items",
            OperationId = "ShoppingList.ListIncomplete",
        ...
        public override async Task<ActionResult<ListIncompleteResponse>> HandleAsync([FromQuery]ListIncompleteRequest request, CancellationToken cancellationToken)
        {
            ...
            if(result.Status == Ardalis.Result.ResultStatus.Ok)
            {
                ...
                response.IncompleteItems = new List<ItemRecord>(
                        result.Value.Select(
                            item => new ItemRecord(item.Id,
                            item.Title,
                            item.Description,
                            item.IsDone)));
            }
